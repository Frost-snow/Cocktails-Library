...
public class SqlRepository<TEntity extends IEntity<Integer>> extends SQLiteOpenHelper implements IRepositoryInt<TEntity> {
            ...
            for (DBColumn column : listColumns){
                ...
                switch (dbMappingAnnotation.dataType()) {
                    case INTEGER:
                    	...
                    	int cIndex = cursor.getColumnIndex(cName);
                    	int ccIndex = cursor.getInt(cIndex);
    ...
    public int getItemsCount() {
    	//Made by Frost - CAUTION!STUPIDNESS AREA!
    	String query = "SELECT * FROM " + getTableName();
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = null;
        try {
    ...
    public TEntity getItem(Integer id) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.query(getTableName(), geColumnsName(), getPrimaryKeyColumn().getName() + "=?", new String[]{id.toString()}, null, null ,null);
